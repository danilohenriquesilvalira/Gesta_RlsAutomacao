...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const { token, profile } = await api.post<LoginResponse>('/api/auth/login', { email, password });
      setToken(token);
      navigate(profile.role === 'admin' ? '/dashboard' : '/meu-dashboard');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'E-mail ou palavra-passe incorretos');
      setLoading(false);
    }
  }
